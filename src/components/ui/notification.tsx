...
type NotificationData = {
  ...
  Request: {
    Pending: { org_name: string; org_id: number }[];
    Completed: string[];
  };
  "What's New": {
...
export default function Notifications({ user }: { user: User }) {
  ...
  const [completedRequests, setCompletedRequests] = useState<
    Array<{ org: string; org_id: number; status: "accepted" | "rejected" }>
  >([]);
  ...
  const handleRequestAction = (
    ...
    setData((prevData) => ({
      ...
      Request: {
        ...prevData.Request,
        Pending: prevData.Request.Pending.filter(
          (item) => item.org_id !== org_id
        ),
      },
    }));
    setCompletedRequests((prev) => [
      ...prev,
      { org, org_id, status: action === "accept" ? "accepted" : "rejected" },
    ]);
  };
...
function RequestSection({
  ...
  title: string;
  requests: { org_name: string; org_id: number }[];
  onAction: (org: string, org_id: number, action: "accept" | "reject") => void;
}) {
  return (
    <div className="rounded-lg border-[1px] border-[#1E293B] w-full">
      <div className="flex p-2 border-b-[1px] border-[#1E293B]">
          ...
          >
            ...
            <div className="flex space-x-2">
              <Button
                ...
                onClick={() =>
                  onAction(request.org_name, request.org_id, "accept")
                }
              >
                Accept
              </Button>
              <Button
                ...
                onClick={() =>
                  onAction(request.org_name, request.org_id, "reject")
                }
              >
                Reject
              </Button>
